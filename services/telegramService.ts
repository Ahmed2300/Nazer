...
👁️ <i>"شد ودان عينيه عليك.. بلاش مرقعة!"</i>
  `;
...
شد ودان (AI) اقترح العقوبات دي.. والجمهور (أو الأدمن) هيختار الأنسب:
...
قررت إدارة "شد ودان" توقيع أقصى عقوبة على الزميل المتخاذل:
...
export const notifyScoreChange = async (token: string, chatId: string, user: User, points: number, reason: string) => {
  ...
  const message = `
${emoji} <b>تحديث في بورصة شد ودان</b>
...
بلاش شغل "الهبد" والمساومة دة.. شد ودان مبيتضحكش عليه.
  `;
  await sendMessage(token, chatId, message);
};
